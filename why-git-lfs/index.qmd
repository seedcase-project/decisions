...
title: "Why Git LFS"
description: |
  Data Packages contain research data that is often large in size and may be sensitive in nature. Storing this data securely and tracking changes over time reliably is essential for good data governance and reproducible research.
  This post explains why we chose Git LFS over other alternatives to manage our data files.
date: "2026-01-28"
categories:
- develop
- data
...
Data Packages often contain large data files, which can be sensitive in
...
Together, these requirements mean that we need to version control data
...
While Git is well suited to version controlling source code, it is not
...
Given that there are multiple tools available for version controlling
large files, the question we need to answer is:

*Which file version control system should we adopt to store and version
control our data files while complying with data privacy policies?*
...
-   Allow files to be stored locally on the secure server without
    pushing data to an external remote (e.g., GitHub)
-   Be easy to install on the secure server without requiring
...
[Git LFS](https://git-lfs.com/) is a widely used system for version
controlling large files within Git-based workflows. It is an open-source
...
-   Does not assume any specific hosting provider by default, so there
    is a lower risk of accidentally pushing data to GitHub.
-   Can be installed without administrative privileges, for example, via
    `conda` on GenomeDK.
...
We decided to use Git LFS because, while all of the options considered
...
-   Because Git LFS is tightly integrated with Git, its default
    ...
    required to prevent sensitive data from being uploaded to GitHub.
-   Even with such safeguards in place, it is not possible to eliminate
    completely the risk of accidental data upload due to human error.
