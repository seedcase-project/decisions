---
title: "Why use fly.io to host a demo of Seedcase"
description: |
...
|                                                              | Azure           | Render                               | Digital Ocean            | Vercel                 | Fly                         |
|--------------------------------------------------------------|-----------------|--------------------------------------|--------------------------|------------------------|-----------------------------|
...
We have decided on using fly.io for our demo and testing needs. Fly is very cheap and the cost is not based on the number of users - only the actual usage (around 1$). It integrates with GitHub authentication and the deployment is easy to setup with a Github action. This part is less user-friendly than **render** and **vercel**, where the deployment happens automatically. However, we would eventually adjust these "magic deployment" anyway to do test and etc before deploying.
...
- Render.com was our initial choice. It was cheap and easy to setup, but the cost is per user plus the usage (around 100$ per month vs 1$ for fly.io). Render.com is very user friendly with magic deployments, but we would need to adjust these anyway for test and etc. 
...
- PostgreSQL is a natural part of fly.io, but it is not a **Managed PostgreSQL** like the other cloud providers. A PostgreSQL is "just an app" in fly.io, so it might require a bit more maintenance. But even if everything is lost (extreme case), then it is okay because fly.io is only used as a demo environment
- The storage is cheap in fly.io, but there are some limitations [Volume considerations](https://fly.io/docs/reference/volumes/#volume-considerations). But again this is only a demo environment
