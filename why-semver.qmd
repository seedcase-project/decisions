...
communicate breaking changes or if new features have been added, or if
...
There are two major software versioning systems, one that bases the
version on what was changed between releases (semantic versioning) and
the other based on when the release was made (calendar versioning).
Broadly, each style has variations, as well as sometimes some
...
-   This is the most widely used versioning system in the software
    industry, so is very familiar to people.
-   It is very clear what has changed between versions, so it conveys
    information about changes well.
-   Has a large number of tools and packages that support it, combine
...
-   Works very well for some software projects, especially those with
    ...
    distribution, Ubuntu, uses a form of CalVer, that increment every 4
    months.
:::
...
software. While it is more difficult to implement and decide on which
...
-   There will also be some variation in how individual team members and
    ...
    There will be some learning within the team based on this.
