...
potential users that might not be familiar with using Github. The
easiest way to do this is by having a website. In order to minimize the
work the team needs to do, we would like to use the files we maintain in
GitHub as the basis for a public website. The question then becomes:
...
writing the documentation for Seedcase itself. We have the following
...
-   Uses Markdown as the basis for writing content.
...
-   There is a good amount of technical knowledge and overhead needed to
    ...
    commands to know to use the tool.
-   Adding extensions and customizing the website requires some
    knowledge and learning to use, nor are there many default or
...
taken over as the tool to use when working with R and data science.
...
-   While not necessary for most purposes, to effectively develop
    features in an extension requires some knowledge of
