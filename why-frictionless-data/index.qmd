...
structure the data and the metadata after it enters into Seedcase
software. So, following our
[guiding principles](https://design.seedcase-project.org/) and
to minimize time spent on tasks that already have solutions, we need to
...
- We need something that has a Python connector or package, since we use
  Python.
...
- [OpenDataDiscovery](https://opendatadiscovery.org/): Requires a Docker
  container to run, mostly focused on commercial needs, and is intended
  as a platform for hosting and sharing (internal) data. There is a
  command-line interface, but there is no easy documentation on how to
  use it, nor is it actively developed.
- [OpenMetadata](https://open-metadata.org): Similar to
  OpenDataDiscovery.
...
surprisingly wasn't listed in the resources above. This is likely for
the opposite reasons we didn't consider the tools they list.
...
- Interacts with multiple programming languages and services, including
  Python and GitHub.
...
- Is still relatively unknown and is not widely used (though, this is a
  problem with all current metadata standards and tools).
...
- [Open Source Data Catalog tools in 2024](https://atlan.com/open-source-data-catalog-tools/)
- [List of Awesome Data Catalog tools](https://github.com/opendatadiscovery/awesome-data-catalogs)
