...
- Be easy to use with Django. Since we use Django and it is a Python
  ...
  or too much JavaScript. Preferably, we want a CSS framework, where CSS
  classes can be directly added to HTML elements.
...
- Has multiple implementations, like
  [Materialize Web](https://materializeweb.com/) and
...
- Relies on [Node.js](https://nodejs.org/en) (via
  [npm](https://en.wikipedia.org/wiki/Npm)) which is an additional
  dependency and needs time to learn to use.
...
- There is better Django integration with Bootstrap, so we might need to
  spend some time properly integrating Material Design
