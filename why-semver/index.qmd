...
- It has a large number of tools and packages that support it, combine
  well with it, or integrate well into development workflows around it.
  For instance, tools like:
  - [Conventional Changelog](https://github.com/conventional-changelog/conventional-changelog)
    to automatically generate changelogs based on the commit messages
    that use the
    [Conventional Commits](/why-conventional-commits/index.qmd) format.
  - [Python Semantic Release](https://python-semantic-release.readthedocs.io/en/latest/)
    to automatically increment the version number based on the changes
    made and create a release based on changes to the version.
...
- It can be very difficult to determine when something is a major,
  minor, or patch change. Sometimes it is perfectly clear, but often, a
  change is hard to place in these three categories.
- It can be difficult to determine what constitutes a breaking change.
- More difficult (but not impossible) to automate, since it can involve
  a large amount of human judgement.
...
- Works very well for some software projects, especially those with very
  regular releases. For instance, the most popular Linux distribution,
  Ubuntu, uses a form of CalVer, that increment every 6 months.
...
- Doesn't work as well for some software projects, especially those that
